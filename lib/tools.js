...
async function translateGoogle(text, targetLang) {
    const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=${targetLang}&dt=t&q=${encodeURIComponent(text)}&ie=UTF-8&oe=UTF-8`;
    await axios({url, timeout: 15000})
        .then(response => {
 ...
 * @returns {String}
 */
function rgbToHex(r, g, b, addHash = true) {
    return (addHash ? '#' : '') + ((1 << 24) + (r << 16) + (g << 8) + b).toString(16).slice(1).toUpperCase();
}
...
function compareVersions(version1, version2) {
    ...
    function compareVersionsArr(version1, version2, result1, result2) {
        for (let index = 0; index < version1.length; index++) {
            const version1Val = parseInt(version1[index]);
            const version2Val = parseInt(index < version2.length ? version2[index] : '0');
 ...
 * @returns {Boolean}
 */
function versionGreaterEquals(version1, version2) {
    const versionDiff = compareVersions(version1, version2);
    return versionDiff === 'higher' || versionDiff === 'equal';
}
 ...
 * @returns {Boolean}
 */
function versionLowerEquals(version1, version2) {
    const versionDiff = compareVersions(version1, version2);
    return versionDiff === 'lower' || versionDiff === 'equal';
}
...
function areObjectsEqual(aObj, bObj, exclude) {
    function doCheck(aObj, bObj) {
        if (typeof aObj === 'undefined' || typeof bObj === 'undefined') {
            return false;
        }
 ...
 * @param {boolean} clear
 */
function cloneObject(input, output, clear = true) {
    if (clear) {
        clearObject(output);
    }
        ...
        Object.keys(input).forEach(prop => {
            if (typeof input[prop] === 'object') {
                if (Array.isArray(input[prop])) {
                    output[prop] = [];
                    cloneArray(input[prop], output[prop]);
                } else {
                    output[prop] = {};
                    cloneObject(input[prop], output[prop], clear);
                }
            } else {
 ...
 * @param {any[]} output
 */
function cloneArray(input, output) {
    clearArray(output);
...
function clearObject(input) {
    Object.keys(input).forEach(item => delete input[item]);
}
