            ...
            axios({
                ...
                .then(response => {
                    if (response.status !== 200)
                        reject('HTTP Error ' + response.statusText + (response.data ? ': ' + JSON.stringify(response.data) : ''));
                    else
                        resolve(response.data);
                })
                .catch(error => {
                    if (error.response && error.response.status !== 200)
                        reject('HTTP Error ' + error.response.status + (error.response.data ? ': ' + JSON.stringify(error.response.data) : ''));
                    else
                        reject(error);
                });
        } catch (e) {
