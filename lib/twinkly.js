...
class Twinkly {
     ...
     * @param {number} code
     */
    translateTwinklyCode(mode, path, code) {
        if (code && code !== HTTPCodes.values.ok)
            return `[${mode}.${path}] (${code}) ${HTTPCodes.text[code]}`;
    }
    ...
    async _getTwinklyCodeCheck(path, headers = {}) {
        ...
        try {
            if (response && typeof response === 'object')
                checkTwinklyCode = this.translateTwinklyCode('GET', path, response['code']);
        } catch (e) {
    ...
    async _postTwinklyCodeCheck(path, data, headers= {}) {
        ...
        try {
            if (response && typeof response === 'object')
                checkTwinklyCode = this.translateTwinklyCode('POST', path, response['code']);
        } catch (e) {
            throw Error(`${e.name}: ${e.message}`);
        }
