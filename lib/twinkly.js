...
class Twinkly {
     ...
     * @param {string} name
     * @param {string} host
     * @param {function(connection: String, functionName: String, key: String, message: String)} handleSentryMessages
     */
    constructor(adapter, name, host, handleSentryMessages) {
         ...
         *         serial: String, production_date: Number, production_site: Number, group: {mode: String, compat_mode: Number}}}
         */
        this.details    = {};
    ...
    async login() {
        ...
        let response;
        try {
            response = await this._postTwinklyCodeCheck('login', {challenge: btoa(getRandomString(32))});
        } catch (e) {
            throw Error(e.message);
        }
    ...
    async verify() {
        ...
        try {
            const response = await this._postTwinklyCodeCheck('verify', {'challenge-response': this.challengeResponse});
    ...
    async getDeviceDetails() {
        ...
        try {
            const param    = tools.versionGreaterEqual('2.8.3', this.firmware) ? '?filter=prod_infos&filter2=group' : '';
            const response = await this._get('gestalt' + param);

            this.getObjectByArray('gestalt', this.details, response, [
                ...
                // added with filter
                'serial', 'production_date', 'production_site', 'group']);
    ...
    async getDeviceName() {
        try {
             ...
             */
            const name = {};
            this.getObjectByArray('device_name', name, response, ['name']);
    ...
    async getTimer() {
        try {
             ...
             */
            const timer = {};
            this.getObjectByArray('timer', timer, response, ['time_now', 'time_on', 'time_off', 'tz']);
    ...
    async getLayout() {
        try {
             ...
             */
            const layout = {};
            this.getObjectByArray('led/layout/full', layout, response,
    ...
    async getLEDMode() {
        try {
             ...
             */
            const mode = {};
            this.getObjectByArray('led/mode', mode, response,
    ...
    async getLEDColor() {
        try {
             ...
             */
            const color = {};
            this.getObjectByArray('led/color', color, response,
    ...
    async getLEDEffects() {
        try {
             ...
             */
            const effects = {};
            this.getObjectByArray('led/effects', effects, response, ['effects_number', 'unique_ids']);
    ...
    async getCurrentLEDEffect() {
        try {
             ...
             */
            const effect = {};
            this.getObjectByArray('led/effects/current', effect, response, ['effect_id', 'preset_id', 'unique_id']);
    ...
    async getLEDConfig() {
        try {
             ...
             */
            const config = {};
            this.getObjectByArray('led/config', config, response, ['strings']);
    ...
    async getLEDMovieConfig() {
        try {
             ...
             */
            const movie = {};
            this.getObjectByArray('led/movie/config', movie, response,
    ...
    async getBrightness() {
        try {
             ...
             */
            const bri = {};
            this.getObjectByArray('led/out/brightness', bri, response, ['value', 'mode']);
    ...
    async getSaturation() {
        try {
            ...
            /** @type {{value: Number, mode: String}} */
            const sat = {};
            this.getObjectByArray('led/out/saturation', sat, response, ['value', 'mode']);
    ...
    async getFirmwareVersion() {
        try {
            ...
            /** @type {{version: String}} */
            const version = {};
            this.getObjectByArray('fw/version', version, response, ['version']);
    ...
    async getListOfMovies() {
        ...
        try {
             ...
             */
            const movies = {};
            this.getObjectByArray('movies', movies, response, ['movies', 'available_frames', 'max_capacity']);
    ...
    async getCurrentMovie() {
        try {
             ...
             */
            const movie = {};
            this.getObjectByArray('movies/current', movie, response, ['id', 'unique_id', 'name']);

            return {movie: movie, code: response['code']};
        } catch (e) {
            // Wenn kein Movie aktiv ist, kommt hier eine Exception (ECONNRESET oder 204 No Content)
    ...
    async getNetworkStatus() {
        try {
             ...
             */
            const status = {};
            this.getObjectByArray('network/status', status, response, ['mode', 'station', 'ap']);
    ...
    async getMqttConfiguration() {
        try {
             ...
             */
            const mqtt = {};
            this.getObjectByArray('mqtt/config', mqtt, response, [
    ...
    /**
     * Get playlist
     * @return {Promise<{playlist: {entries: {id: Number, handle: Number, name: String, unique_id: String, duration: Number}[]}, code: Number}>}
    ...
    async getPlaylist() {
        ...
        try {
            ...
            /**
             * @type {{entries:{id: Number, handle: Number, name: String, unique_id: String, duration: Number}[]}}
             */
            const playlist = {};
            this.getObjectByArray('playlist', playlist, response, ['entries']);
     ...
     * @return {Promise<{playlist : {id: Number, unique_id: String, name: String}, code: Number}>}
    ...
    async getCurrentPlaylistEntry() {
        try {
             ...
             */
            const playlist  = {};
            this.getObjectByArray('playlist/current', playlist , response, ['id', 'unique_id', 'name']);
     ...
     */
    getObjectByArray(name, output, input, include = [], exclude = ['code']) {
        for (const key of Object.keys(input)) {
            if (include.length === 0 || include.includes(key))
                output[key] = input[key];
            else if (!exclude.includes(key)) {
                this.handleSentryMessages(this.name, 'getObjectByArray',
    ...
    async checkDetailInfo(filter) {
        ...
        return Object.keys(this.details).includes(filter.name) && this.details[filter.name] === filter.val;
    }
    ...
    async _get(path, headers = {}) {
        ...
        try {
            // GET ausführen...
            return await this._getTwinklyCodeCheck(path, headers);
        } catch (e) {
    ...
    /**
     * GET and check Twinkly code
     * @private
     * @param {string} path
     * @param {{}} headers
     * @return {Promise<{}>}
     */
    async _getTwinklyCodeCheck(path, headers = {}) {
        const getHeaders = {};
        this._addHttpHeaders(headers, getHeaders);

        let response;
        try {
            response = await request.sendGetHTTP(this.adapter, this.base() + '/' + path, getHeaders);
        } catch (e) {
        ...
        try {
            this._checkResponseTwinklyCode('GET', path, response);
        } catch (e) {
            throw Error(e.message);
        }
    ...
    async _post(path, data, headers = {}) {
        ...
        try {
            // POST ausführen...
            return await this._postTwinklyCodeCheck(path, data, headers);
        } catch (e) {
            if (e.message !== HTTPCodes.INVALID_TOKEN)
                throw Error(e.message);
     ...
     * @return {Promise<{}>}
     */
    async _postTwinklyCodeCheck(path, data, headers= {}) {
        const postHeaders = {};
        this._addHttpHeaders(headers, postHeaders);

        let response;
        try {
            response = await request.sendPostHTTP(this.adapter, this.base() + '/' + path, data, postHeaders);
        } catch (e) {
            if (e.message && String(e.message).includes(HTTPCodes.INVALID_TOKEN))
                throw Error(HTTPCodes.INVALID_TOKEN);
            else
        ...
        try {
            this._checkResponseTwinklyCode('POST', path, response);
        } catch (e) {
            throw Error(e.message);
        }
    ...
    async _delete(path, headers = {}) {
        ...
        try {
            // POST ausführen...
            return await this._deleteTwinklyCodeCheck(path, headers);
        } catch (e) {
            if (e.message !== HTTPCodes.INVALID_TOKEN)
                throw Error(e.message);
     ...
     * @return {Promise<{}>}
     */
    async _deleteTwinklyCodeCheck(path, headers= {}) {
        const deleteHeaders = {};
        this._addHttpHeaders(headers, deleteHeaders);

        let response;
        try {
            response = await request.sendDeleteHTTP(this.adapter, this.base() + '/' + path, deleteHeaders);
        } catch (e) {
            if (e.message && String(e.message).includes(HTTPCodes.INVALID_TOKEN))
                throw Error(HTTPCodes.INVALID_TOKEN);
            else
        ...
        try {
            this._checkResponseTwinklyCode('DELETE', path, response);
        } catch (e) {
            throw Error(e.message);
        }
