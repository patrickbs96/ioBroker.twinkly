...
class Twinkly {
    ...
    constructor(adapter, name, host, handleSentryMessages) {
        ...
        this.details  = {};
        this.firmware = '';
        this.mode     = lightModes.value.off;
        this.effects  = {};
        this.movies   = {};
    }
    ...
    async getDeviceDetails() {
        try {
             ...
             */
            const details = {};
            this.getObjectByArray('gestalt', details, response, [
    ...
    async getListOfLEDEffects() {
        ...
        try {
            ...
            for (let effect = 0; effect < response.effects.effects_number; effect++) {
                this.effects[effect] = `Effect ${effect+1}`;
            }
        } catch (e) {
    ...
    async getListOfMovies() {
        ...
        try {
            ...
            for (const movie of response.movies) {
                this.movies[movie.id] = movie.name;
            }
    ...
    async sendGetHTTP(url, headers = {}) {
        try {
            return await request.sendGetHTTP(this.adapter, url, headers);
        } catch (e) {
    ...
    async _post(path, data, headers = {}) {
        ...
        try {
            // POST ausführen...
            return await this._doPOST(path, data, headers);
        } catch (e) {
            if (e.message !== HTTPCodes.INVALID_TOKEN)
                throw Error(e.message);
    ...
    async sendPostHTTP(url, body, headers = {}) {
        try {
            return await request.sendPostHTTP(this.adapter, url, body, headers);
        } catch (e) {
            if (e && e.message && String(e.message).includes(HTTPCodes.INVALID_TOKEN))
                throw Error(HTTPCodes.INVALID_TOKEN);
            else
