...
class Connection {
    ...
    async get_network_status() {
        ...
        return new Promise((resolve, reject) => {
            if (resultError)
                ...
                resolve({status: this.getObjectByArray('network/status', response), code: response['code']});
        });
    }
     ...
     * @return {Promise<{code: Number}>}
     */
    async set_mqtt(broker_host, broker_port, client_id, user, keep_alive_interval) {
        let resultError;
