...
class Twinkly {
    ...
    async sendRealtimeFrame(frame) {
        const response = await this._post('led/rt/frame', frame, {'Content-Type': 'application/octet-stream'});
        return {code: response['code']};
    }
    ...
    async setCurrentMusicDriver(driverName) {
        ...
        const response = await this._post('music/drivers/current', {unique_id: uniqueId});
        return {code: response['code']};
    }
    ...
    async setNextMusicDriver() {
        const response = await this._post('music/drivers/current', {action: 'next'});
        return {code: response['code']};
    }
    ...
    async setPreviousMusicDriver() {
        const response = await this._post('music/drivers/current', {action: 'prev'});
        return {code: response['code']};
    }
    ...
    async setMusicOn(enable) {
        const response = await this._post('music/enabled', {enabled: enable ? 1 : 0});
        return {code: response['code']};
    }
    ...
    async _handleHttpTwinklyCodeCheck(mode, path, data, headers= {}) {
        ...
        let response;
        try {
            response = await this._sendRequest(this.base() + path, data, mode, httpHeaders);
        } catch (e) {
    ...
    async _sendRequest(url, body, method, headers = {}) {
        ...
        this.adapter.log.debug(`[_sendRequest.${method}] url="${url}", body="${body !== undefined ? JSON.stringify(body) : 'null'}", headers="${JSON.stringify(headers)}"`);
        ...
        try {
            ...
            this.adapter.log.debug(`[_sendRequest.${method}] ${e.name}: "${e.message}", ${message}`);
        ...
        if (response.status !== 200) {
            ...
            if (response.data) {
                this.adapter.log.debug(`[_sendRequest.${method}.response] ${JSON.stringify(response.data)}`);
            }
