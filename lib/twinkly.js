...
class Connection {
    ...
    async set_timer(data) {
        ...
        if (!resultError)
            result = await this._post('timer', data).catch(error => {resultError = error;});
     ...
     *                             fw_family: String, bytes_per_led: Number},
     *                   code: Number}>}
     */
    async get_details() {
        let resultError;
            ...
            if (resultError)
                ...
                resolve({
                    details: this.getObjectByArray('gestalt', response, [
                        ...
                        'max_supported_led', 'wire_type',       'measured_frame_rate', 'uuid',           'fw_family', 'bytes_per_led']),
                    code: response['code']});
        });
    }

    /**
     * @return {Promise<{mode: {mode: String, shop_mode: Number}, code: Number}>}
     */
    async get_mode() {
        let resultError;
        ...
        return new Promise((resolve, reject) => {
            if (resultError)
                ...
                resolve({mode: this.getObjectByArray('led/mode', response, ['mode', 'shop_mode']), code: response['code']});
        });
    }
    ...
    async set_mqtt(data) {
        ...
        if (!resultError)
            result = await this._post('mqtt/config', data).catch(error => {resultError = error;});
    ...
    getObjectByArray(name, input, include = [], exclude = ['code']) {
        ...
        for (const key of Object.keys(input)) {
            if (include.length === 0 || include.includes(key))
                ...
                this.log.warn(`[getObjectByArray] New Item <${key}, ${input[key]}> in Response ${name}! Please notify the developer!`);
        }
    ...
    sendGetHTTP(url, headers = {}) {
        ...
        return new Promise((resolve, reject) => {
            request.sendGetHTTP(url, headers)
                .then(response => {
                    if (response) oThis.log.debug('[sendGetHTTP] ' + JSON.stringify(response));
                    resolve(response);
                })
                .catch(error => {
    ...
    sendPostHTTP(url, body, headers = {}) {
        ...
        return new Promise((resolve, reject) => {
            request.sendPostHTTP(url, body, headers)
                .then(response => {
                    if (response) oThis.log.debug('[sendPostHTTP] ' + JSON.stringify(response));
                    resolve(response);
                })
                .catch(error => {
                    if (error && error.includes(HTTPCodes.INVALID_TOKEN))
                        reject(HTTPCodes.INVALID_TOKEN);
                    else
                        reject(error);
                });
