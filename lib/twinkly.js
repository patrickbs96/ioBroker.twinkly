...
class Twinkly {
     ...
     */
    constructor(adapter, name, host, onModeChange) {
     ...
     * @param {{r: Number, g: Number, b: Number}|String|{r: Number, g: Number, b: Number}[]|String[]} color RGB-Object or Hex-String
     * @returns {{r: Number, g: Number, b: Number}[]}
     */
    generateFullFrame (color) {
     ...
     * @param {{r: Number, g: Number, b: Number}[]|String[]} colors RGB-Object or Hex-String
     * @returns {{r: Number, g: Number, b: Number}[]}
     */
    generateFullFrames (colors) {
        const frames = [];
        if (tools.isArray(colors)) {
            for (const color of colors) {
                frames.push(this.generateFullFrame(color));
            }
        } else {
            frames.push(this.generateFullFrame(colors));
        }
    ...
    async _handleHttpTwinklyCodeCheck(mode, path, data, headers= {}) {
        ...
        try {
            switch (mode) {
                case 'GET'   : response = await request.sendGetHTTP(this.adapter, this.base() + '/' + path, httpHeaders); break;
                case 'POST'  : response = await request.sendPostHTTP(this.adapter, this.base() + '/' + path, data, httpHeaders); break;
                case 'DELETE': response = await request.sendDeleteHTTP(this.adapter, this.base() + '/' + path, httpHeaders); break;
            }
        } catch (e) {
