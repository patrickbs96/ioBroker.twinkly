...
class Connection {
    ...
    async setLEDColorRGBW(red, green, blue, white) {
        ...
        try {
            ...
            if (await this.checkDetailInfo({name: 'led_profile', val: 'RGBW'}) && white)
                data['white'] = white;
     ...
     */
    async setCurrentLEDEffect(color) {
        // try {
        //     const response = await this._post('led/mode', {'mode': mode});
        //
        //     return {code: response['code']};
    ...
    async sendGetHTTP(url, headers = {}) {
        try {
            ...
            if (e && typeof e === 'string' && String(e).includes(HTTPCodes.INVALID_TOKEN))
