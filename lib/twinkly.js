...
class Twinkly {
    ...
    async getDeviceDetails() {
        ...
        try {
            ...
            if (Object.keys(oldDetails).length > 0) {
                ...
                if (this.details.group && (!oldDetails.group || oldDetails.group.mode !== this.details.group.mode)) {
                    if (this.onDataChange) {
                        this.onDataChange(this.name, 'groupMode', this.details.group.mode, oldDetails.group ? oldDetails.group.mode : '');
                    }
                }
            }
    ...
    async getLEDMode() {
        try {
            ...
            if (this.ledMode !== mode.mode && mode.mode !== '') {
                const oldMode = this.ledMode;
                this.ledMode  = mode.mode;

                if (oldMode !== '' && this.onDataChange) {
                    await this.onDataChange(this.name, 'ledMode', this.ledMode, oldMode);
                }
            }
    ...
    async getFirmwareVersion() {
        try {
            ...
            if (this.firmware !== firmware.version) {
                const oldFirmware = this.firmware;
                this.firmware     = firmware.version;

                if (this.onDataChange) {
                    await this.onDataChange(this.name, 'firmware', this.firmware, oldFirmware);
                }
            }
