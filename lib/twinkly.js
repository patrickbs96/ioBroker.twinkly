...
class Connection {
    ...
    async getLEDColor() {
        let resultError;
        const response = await this._get('led/color').catch(error => {resultError = error;});

        return new Promise((resolve, reject) => {
            if (resultError)
                reject(resultError);
            else
                resolve({color: this.getObjectByArray('led/color', response,
    ...
    async setLEDColorHSV(hue, sat, value) {
        let resultError;
        const response = await this.setLEDColor({hue: hue, saturation: sat, value: value}).catch(error => {resultError = error;});
     ...
     * @return {Promise<{code: Number}>}
     */
    async setLEDColorRGB(red, green, blue) {
        let resultError;
        const response = await this.setLEDColor({red: red, green: green, blue: blue}).catch(error => {resultError = error;});
    ...
    async setLEDColor(color) {
        let resultError;
        const response = await this._post('led/color', color).catch(error => {resultError = error;});
