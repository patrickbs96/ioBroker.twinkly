...
class Twinkly {
     ...
     * @param {function(connection: String, oldMode: String, newMode: String)} onModeChange
     */
    constructor(adapter, name, host, handleSentryMessages, onModeChange) {
    ...
    async getDeviceDetails() {
        ...
        try {
            ...
            const response = await this._getTwinklyCodeCheck('gestalt' + param);
            ...
            this.getObjectByArray('gestalt', this.details, response, [
                ...
                'max_movies', 'max_supported_led', 'wire_type', 'measured_frame_rate', 'uuid', 'fw_family',
    ...
    async getLEDMode() {
        try {
            ...
            if (this.ledMode !== mode.mode) {
                await this.onModeChange(this.name, this.ledMode, mode.mode);
    ...
    async getFirmwareVersion() {
        try {
            const response = await this._getTwinklyCodeCheck('fw/version');
    ...
    async getListOfMovies() {
        ...
        try {
             ...
             */
            const movies = {};
            this.getObjectByArray('movies', movies, response, ['movies', 'available_frames', 'max', 'max_capacity']);
    ...
    async getCurrentPlaylistEntry() {
        try {
             ...
             */
            const playlist  = {};
            this.getObjectByArray('playlist/current', playlist , response, ['id', 'unique_id', 'name', 'duration']);
